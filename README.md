...
- **记忆系统** — Token 预算驱动 FIFO 淘汰 + 中文分词检索 + 时间衰减排序 + 跨会话持久化 + 同 key 覆盖去重
- **上下文压缩** — Map-Reduce 策略（MemoryEntry 压缩）+ 对话历史压缩（LLM 调前评估，user 边界分割，原地替换）
- **事实提取** — 对话结束自动提取关键事实（只提取用户消息 + 三层输出过滤 + key 级去重），支持 `/save <事实>` 手动存储
- **Plan-and-Execute 增强** — 每任务 mini ReAct 多轮工具调用 + PlanReviewHandler 用户审查 + ByteArrayOutputStream 并行输出防交错
- **Multi-Agent** — Team 模式下 Planner → Workers → Reviewer 三角色分工协作，并行执行 + 审查重试
- **RAG 代码检索** — 代码库索引（Java AST 分块 + 非 Java 行级分块）+ 混合检索（语义 + 关键词 + 类型加分）
...
| **Plan-and-Execute** | `/plan` | 创建项目、多文件操作、编译运行 |
| **Team**（多 Agent） | `/team` | 复杂多步骤任务，需分工协作+质量审查 |

复杂任务自动判断规则（ReAct 模式下）：输入长度 > 50 字符，或包含 3 个以上动作关键词。
...
MemoryManager（门面）
  ...
  ├─ LongTermMemory：长期记忆，同 key 覆盖 + 跨会话持久化（JSON）
  ├─ ContextCompressor：Map-Reduce 压缩 + 事实提取（输入源过滤 + 三层输出过滤）
  ├─ ConversationHistoryCompactor：对话历史原地压缩（LLM 调前评估，user 边界分割）
  ├─ TokenBudget：预算分配器（窗口 / 系统 / 工具 / 回复）
  └─ MemoryRetriever：分词检索 + 时间衰减 + 来源加权
```

- **中文分词**：双字滑动窗口 + 单字重叠匹配，支持模糊中文检索
...
| `/thinking` | 切换思考模式（默认关闭，/thinking 开启深度思考）|
| `/effort` | 查看当前思考强度 |
| `/effort high/max` | 设置思考强度 |
| `/memory` | 查看记忆系统状态（短期/长期/Token 统计）|
| `/save` | 自动提取事实到长期记忆 |
| `/save <事实>` | 手动保存一条事实（不走 LLM） |
| `/index <路径>` | 索引代码库到 RAG 向量存储 |
| `/search <查询>` | 混合检索代码（语义 + 关键词） |
...
│   ├── ContextCompressor.java     # Map-Reduce 压缩 + 过滤式事实提取
│   ├── ConversationHistoryCompactor.java  # 对话历史原地压缩（LLM 调前触发）
...
用户输入
  ...
  └─ 复杂任务 → PlanAndExecuteAgent
        ...
        │    └─ 第二层：逐阶段调用 LLM 细化子任务
        ├─ 审查阶段 → PlanReviewHandler 用户审查（y=执行 / n=取消 / 反馈补充）
        ├─ 执行阶段
        │    ├─ 拓扑排序 → 计算 DAG 执行顺序
        │    ├─ 每任务 mini ReAct（最多 5 轮工具调用，LLM 自主结束）
        │    ├─ 并行（FixedThreadPool + ByteArrayOutputStream 防交错）
        │    ├─ 自我修正（失败数 ≥2 且失败率 >50% 则重新规划）
        │    └─ 进度条 + 状态图标实时刷新
        ├─ 总结阶段 → LLM 汇总执行结果
        └─ writeBackToContext() → 共享上下文
```
